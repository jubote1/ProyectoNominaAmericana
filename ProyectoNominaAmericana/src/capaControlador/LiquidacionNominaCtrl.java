...
public class LiquidacionNominaCtrl {
	
	public static void main(String args[])
	{
		LiquidacionNominaCtrl liqNomina = new LiquidacionNominaCtrl();
		liqNomina.enviarCorreoLiquidacionNomina("2021-10-II");
	}
	...
	public void enviarCorreoLiquidacionNomina(String quincenaPago)
	{
		...
		for(int i = 0;  i < nominasLiquidadas.size(); i++)
		{
			...
			if(liqNomina.getCorreoElectronico().length() > 0)
			{
				...
				correo.setAsunto("Liquidacion Nomina" + liqNomina.getQuincenaPago());
				correo.setContrasena(infoCorreo.getClaveCorreo());
				ArrayList correos = new ArrayList();
				correos.add(new String(liqNomina.getCorreoElectronico()));
				String strLiquidacionNomina = generarHtmlLiquidacionNomina(liqNomina);
				//En este punto vamos a intervenir para generar un PDF con el HTML
				try 
				{
				    FileOutputStream file = new FileOutputStream(new File("C:\\reportes\\" + liqNomina.getCedula() + ".pdf"));
				...
				correo.setMensaje("Hola " + liqNomina.getNombre() + " " + liqNomina.getApellido() + " en el archivo anexo a continuación encontrarás la liquidación de tu nomina : " + liqNomina.getQuincena() + "\n");
